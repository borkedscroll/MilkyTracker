/*
 *  tracker/PianoRollControlEventListener.cpp
 *
...
/*
 *  PianoRollControlEventListener.cpp
 *  MilkyTracker
 *
...
pp_int32 PianoRollControl::dispatchEvent(PPEvent* event)
{ 
	if (pattern == NULL)
		return -1;
...
void PianoRollControl::selectionModifierKeyDown()
{
...
void PianoRollControl::selectionModifierKeyUp()
{
	keyboardStartSelection = false;
	if (moveSelection)
		parentScreen->paintControl(this);
...
pp_int32 PianoRollControl::handleEvent(PPObject* sender, PPEvent* event)
{
...
pp_int32 PianoRollControl::pointInChannelHeading(PPPoint& cp)
{
	if (cp.x < 0)
		return -1;
...
void PianoRollControl::invokeMenu(pp_int32 channel, const PPPoint& p)
{
...
bool PianoRollControl::isSoloChannel(pp_int32 c)
{
	pp_int32 i = 0;
