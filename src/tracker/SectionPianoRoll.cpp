...
class DialogResponderPianoRoll : public DialogResponder
{
private:
	SectionPianoRoll& section;
	
public:
	DialogResponderPianoRoll(SectionPianoRoll& section) :
		section(section)
	{
	}
...
SectionPianoRoll::SectionPianoRoll(Tracker& theTracker) :
	SectionAbstract(theTracker, NULL, new DialogResponderPianoRoll(*this)),
	containerEntire(NULL),
	visible(false),
	sampleEditorControl(NULL),
	pianoRollControl(NULL),
...
pp_int32 SectionPianoRoll::handleEvent(PPObject* sender, PPEvent* event)
{
...
void SectionPianoRoll::init()
{
	init(0, tracker.MAXEDITORHEIGHT()-tracker.SAMPLESECTIONDEFAULTHEIGHT());
}

void SectionPianoRoll::init(pp_int32 x, pp_int32 y)
{
	PPScreen* screen = tracker.screen;
	...
	// maybe add PianoRollControl here (modified patterneditor)?
	...
	// PianoRollControl
	pianoRollControl = new PianoRollControl(PATTERN_EDITOR, screen, this, PPPoint(x+2, y+2), PPSize(sampleEditorContainer->getSize().width-4, sampleEditorContainer->getSize().height-4));
	pianoRollControl->attachPatternEditor(tracker.moduleEditor->getPatternEditor());
	sampleEditorContainer->addControl(pianoRollControl);
	containerEntire->addControl(sampleEditorContainer);
	...
	container->addControl(new PPStaticText(STATICTEXT_DISPLAY, NULL, NULL, PPPoint(x2 + 2 + 2, y2 + 4), screen->getWidth() < 800 ? "Displ." : "Piano Roll!", true));		
...
void SectionPianoRoll::showSection(bool bShow)
{
	containerEntire->show(bShow);
}

void SectionPianoRoll::realign()
{
...
void SectionPianoRoll::show(bool bShow)
{
#ifdef __LOWRES__
	PPScreen* screen = tracker.screen;
...
void SectionPianoRoll::update(bool repaint/* = true*/)
{
	realUpdate(repaint, false, true);
}

void SectionPianoRoll::notifyTabSwitch()
{
	if (isVisible())
		realign();
}

// No need to reattach pianoroll again, because reattaching also clears out selection
void SectionPianoRoll::refresh(bool repaint/* = true*/)
{
	realUpdate(repaint, false, false);
}

void SectionPianoRoll::realUpdate(bool repaint, bool force, bool reAttach)
{
	if (!initialised)
		return;
...
void SectionPianoRoll::updateSampleWindow(bool repaint/* = true*/)
{
	PPScreen* screen = tracker.screen;
	// screen->paintControl(sampleEditorControl, false);
	if (repaint)
...
void SectionPianoRoll::updateAfterLoad()
{
...
SampleEditorControl* SectionPianoRoll::getSampleEditorControl(bool forceAttach/* = true*/) 
{
	if (forceAttach && 
		sampleEditorControl && 
...
void SectionPianoRoll::resetSampleEditor()
{
	if (sampleEditorControl)
		sampleEditorControl->reset();
}

bool SectionPianoRoll::isEmptySample()
{
	return !tracker.getSampleEditor()->isEditableSample();
}

void SectionPianoRoll::setOffsetFormat(pp_uint32 offsetFormat)
{
...
void SectionPianoRoll::toggleOffsetFormat()
{
...
void SectionPianoRoll::setOffsetText(pp_uint32 ID, pp_uint32 offset)
{
	switch (offsetFormat)
	{
...
// void SectionPianoRoll::handleClearSample()
// {
// 	SampleEditor* sampleEditor = sampleEditorControl->getSampleEditor();
...
// void SectionPianoRoll::handleCropSample()
// {
// 	SampleEditor* sampleEditor = sampleEditorControl->getSampleEditor();
...
// void SectionPianoRoll::handleMinimizeSample()
// {
// 	SampleEditor* sampleEditor = sampleEditorControl->getSampleEditor();
...
// void SectionPianoRoll::handleConvertSampleResolution()
// {
// 	SampleEditor* sampleEditor = sampleEditorControl->getSampleEditor();
// 	if (sampleEditor->isEditableSample())
// 	{
